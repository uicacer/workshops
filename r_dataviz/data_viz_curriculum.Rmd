...
**Data:** Always start with the data, identify the dimensions you want
to visualize.\
**Aesthetics:** Confirm the axes based on the data dimensions, positions
of various data points in the plot. Also check if any form of encoding
is needed including size, shape, color and so on which are useful for
plotting multiple data dimensions.\
**Scale:** Do we need to scale the potential values, use a specific
scale to represent multiple values or a range?\
**Geometric objects:** These are popularly known as 'geoms'. This would
cover the way we would depict the data points on the visualization.
Should it be points, bars, lines and so on?\
...
![hierarchy grammar of graphics](gg_hierarchy.png) source:
...
ggplot(data = animals, aes(x = weight, y = hindfoot_length)) + geom_point()
```
...
surveys_plot <- ggplot(data = animals,
...
-   You may notice that we sometimes reference 'ggplot2' and sometimes
    'ggplot'. To clarify, 'ggplot2' is the name of the most recent
    version of the package. However, any time we call the function
    itself, it's just called 'ggplot'.
...
> data sets with large numbers of observations, such as the `animals`
...
> fall within its boundaries. To use hexagonal binning with
...
ggplot(data = animals, mapping = aes(x = species_id, y = hindfoot_length)) +
       geom_boxplot() +
...
sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-visualization-2.1.pdf),
...
ggplot(animals, aes(x = species_id, y = hindfoot_length)) +
    geom_boxplot() +
    grey_theme
```
...
>     [https://www.cookbook-r.com/Graphs/Colors\_(ggplot2)/](https://www.cookbook-r.com/Graphs/Colors_(ggplot2)/){.uri}).
...
plot_count <- ggplot(data = yearly_counts, aes(x = year, y = n)) +
  geom_line(aes(color = genus)) +
